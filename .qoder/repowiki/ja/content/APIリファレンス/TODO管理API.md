...
## はじめに
本ドキュメントは、高度なTodo管理APIのエンドポイントを網羅的にドキュメント化することを目的としています。新たに追加された高度な検索・フィルタリング・ページネーション機能、優先度管理、期限日設定、タグフィルタリング機能を含む、完全に拡張されたAPI仕様について、HTTPメソッド、URLパターン、クエリパラメータ、リクエストボディスキーマ、レスポンススキーマ、認証方式、UUIDベースのID管理、エラーレスポンス形式、バリデーションエラー、データベースエラー対処法、API呼び出し例、クライアント実装のベストプラクティス、パフォーマンス最適化のヒント、リアルタイム更新機能を示します。
...
**更新** 高度な検索・フィルタリング・ページネーション機能が追加されました。
...
- **クエリパラメータ**：
  ...
  - priority（PriorityEnum、オプション）：優先度でのフィルタ（HIGH/MEDIUM/LOW）
  - tags（string、オプション）：タグでのフィルタ（カンマ区切り複数指定可）
  - sort_by（string、デフォルト："created_at"）：ソート対象（created_at/priority/due_date）
  - sort_order（string、デフォルト："desc"）：ソート順（asc/desc）
- **パスパラメータ**：なし
- **リクエストボディ**：なし
